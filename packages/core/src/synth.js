...
export function expRamp(t, tMax, v0, v1) {
  if (Math.sign(v0) == -Math.sign(v1)) return v0;
...
KrajeskiFilter.prototype.apply = function (s, cutoff, resonance) {
  ...
  for (var s = 0; s < n; ++s) {
    ...
    for (var i = 0; i < 4; i++) {
      state[i + 1] =
        g * ((0.3 / 1.3) * state[i] + (1 / 1.3) * delay[i] - state[i + 1]) +
        state[i + 1];
      delay[i] = state[i];
    }
...
KrajeskiFilter.prototype.setResonance = function (resonance) {
  ...
  this.gRes =
    resonance *
    (1.0029 + 0.0526 * wc - 0.926 * Math.pow(wc, 2) + 0.0218 * Math.pow(wc, 3));
};
...
KrajeskiFilter.prototype.setCutoff = function (cutoff) {
  ...
  this.g =
    0.9892 * wc -
    0.4342 * Math.pow(wc, 2) +
    0.1381 * Math.pow(wc, 3) -
    0.0202 * Math.pow(wc, 4);
};
